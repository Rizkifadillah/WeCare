...
<div class="row">
    <div class="col-lg-7">
        ...
        <x-card>
            ...
            <x-slot name="footer">
                <h3 class="font-weight-bold">Rp. {{format_uang($campaign->nominal - $campaign->cashouts->whereIn('status', ['success', 'pending'])->sum('cashout_amount'))}}</h3>
                ...
                <div class="progress" style="height: .3rem;">
                    <div class="progress-bar" role="progressbar" 
                        ...
                        aria-valuenow="{{ $campaign->nominal / $campaign->goal * 100 }}" 
        ...
        <x-card class="pre">
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    ...
                    <input type="text" class="form-control-plaintext bank-account" value="{{sembunyikan_text($bank->pivot->account,2) ?? ''}}" readonly>
                </div>
                <div class="col-lg-4">
                    <p class="mb-0 font-weight-bold">Nama pemilik:</p>
                    <input type="text" class="form-control-plaintext bank-ownername" value="{{ sembunyikan_text($bank->pivot->name,2) ?? ''}}" readonly>
                </div>
                <div class="col mt-3">
                    @if ($bank)
                    <button class="btn btn-info float-left" data-toggle="modal" data-target="#ganti-rekening">Ganti Rekening Tujuan</button>                        
                    @else
    ...
    <div class="col-lg-5">
       <h3 class="text-primary">Yang bisa dicairkan: 
           <p>
               <strong>
                   Rp. {{format_uang($campaign->nominal - $campaign->cashouts->whereIn('status', ['success', 'pending'])->sum('cashout_amount'))}}
                </strong>
            </p>
        </h3>
       ...
       <div class="alert alert-light border-primary">
           Disarankan untuk melakukan melakukan pencairan dana pada jam kerja normal(Senin-Jumat 08.00-20.00) untuk menghindari transaksi pending dikarenakan terkena cut off time dari bank yang bersangkutan.
       </div>
       <x-card>
           <form action="{{route('campaign.cashout.store', $campaign->id)}}" method="post" class="form-pencairan" onsubmit="reviewCashout()">
                ...
                <input type="hidden" value="{{ $campaign->user_id }}" name="user_id">
                <input type="hidden" value="{{ $bank->id ?? '' }}" name="bank_id">
                <input type="hidden" value="{{ $campaign->nominal - $campaign->cashouts->whereIn('status', ['success', 'pending'])->sum('cashout_amount') }}" name="total">
...
<x-modal size='modal-md' id="ganti-rekening">
    ...
    <p class="m-3">Silahkan klik link <a href="{{ route('profile.show')}}?pills=bank">berikut ini</a> untuk update rekening tujuan</p>
</x-modal>
