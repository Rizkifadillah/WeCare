...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item"><a href="{{ route('donation.index') }}">Pencairan</a></li>
...
<div class="row">
    <div class="col-lg-7">
        ...
        <div class="pre alert alert-light border-secondary text-info">
        ...
        <x-card class="card-confirm">
            <x-slot name="header">
                <h5 class="mb-0">
                    Dibuat pada {{ tanggal_indonesia($cashout->created_at)}} {{date('H:i', strtotime($cashout->created_at))}}
                </h5>
            </x-slot>
            ...
            @else
                <div class="alert alert-light border-danger text-danger mb-0">
                    Nominal yang ingin Anda cairkan adalah sebesar Rp. {{format_uang($cashoutcashout_amount)}}
                </div>
            @endif
                ...
                @switch($cashout->status)
                    @case('pending')
                        ...
                        <button class="btn btn-secondary float-left"
                            onclick="editForm('{{ route('cashout.update', $cashout->id) }}', 'cenceled', 'Yakin ingin membatalkan pencairan terpilih?', 'secondary')">Batalkan</button> 
                        @endif
    ...
    function submitForm(originalForm) {
        $.post({
            ...
            .done(response => {
                ...
                else if (response.data.status == 'cenceled') color = 'danger';
